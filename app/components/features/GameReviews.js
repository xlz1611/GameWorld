...
/**
 * 游戏评论组件
 * 用于在游戏详情页面中显示用户评论和评分统计，以及允许用户发表新评论
 * 
 * @param {string} gameId - 游戏ID，用于存储和加载评论数据
 * @returns {JSX.Element} 游戏评论组件
 */
const GameReviews = ({ gameId }) => {
  ...
  const loadReviews = () => {
    try {
      const savedReviews = localStorage.getItem(`game_reviews_${gameId}`)
      if (savedReviews) {
  ...
  const saveReviews = (updatedReviews) => {
    try {
      localStorage.setItem(`game_reviews_${gameId}`, JSON.stringify(updatedReviews))
    } catch (error) {
  ...
  const handleSubmitReview = () => {
    ...
    const review = {
      ...
      content: sanitizeInput(newReview.trim()),
      rating: userRating,
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-center md:text-left">
            ...
            <p className="text-muted text-sm">
              基于 {reviews.length > 0 ? reviews.length : 123} 条用户评价
            </p>
          </div>
              ...
              return (
                <div key={rating} className="flex items-center gap-3 mb-2">
                  <div className="w-16 flex justify-end">
                    <span className="text-sm text-muted">{rating}星</span>
                  </div>
                  <div className="flex-1 bg-[#1E293B] rounded-full h-2">
                    <div
                      className="bg-brand h-2 rounded-full"
                      style={{ width: `${percentage}%` }}
                      aria-label={`${rating}星评价占比 ${percentage.toFixed(0)}%`}
                    />
                  </div>
          ...
          <div className="space-y-4" aria-live="polite">
            {reviews.map((review) => (
              <div key={review.id} className="bg-[#0F172A] rounded-xl p-6">
                ...
                <p className="text-muted leading-relaxed" dangerouslySetInnerHTML={{ __html: review.content }}></p>
              </div>
            ))}
          </div>
        )}
